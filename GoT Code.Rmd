...
# Part 6: Authorial Studies

``` {r}
# Examining the top 3 rated directors in Game of Thrones: is there a statistically significant difference in terms of rating if they did/didn't direct an episode?
...
``` {r}
# to test methodology
test <- ddply(Thrones, .(director), summarize,  rating=mean(DandD))
```
...
clearly a dropoff after episode 40.

``` {r}
# What is the biggest factor in terms of episode quality - character score (how many characters that people like are in an episode), whether or not an episode was directed by an outside talent (not the writers of the show themselves), whether or not book material was still there to guide the writing team (as defined by article)?
```
...
Clearly, D and D are the greatest factor in terms of rating. 