...
for(i in 1:73){
  ...
  Thrones$credits[i] <- read_html(link) %>% html_nodes("fullcredits_content.name a") %>%
    html_text %>% paste(collapse = "\n\n")
}
```
...
Textfreq <- ggplot(data=top_df2, aes(x=reorder(word, -freq), y=freq)) + geom_bar(stat="identity") + xlab("Keywords") + ylab("Frequency") + theme(legend.title = element_text(size = 3), legend.text = element_text(size = 0.01)) + ggtitle("Most Frequent Game of Thrones Synopsis Keywords")
Textfreq
```
